...
/**
 * The expanding-card workout screen's tests, kept as a checklist.
 *
 * The screen is gone, so these cannot pass — but almost none of the
 * *behaviours* are gone: they have to reappear through the focus flow's own
 * controls. The table in the plan tracks which task restores each one, and this
 * file is deleted in task 7.2 once every row is ticked.
 *
 * Skipped rather than deleted on purpose: a deleted test is a behaviour nobody
 * remembers losing.
 */
describe.skip('SessionScreen (legacy, replaced by the focus flow)', () => {
